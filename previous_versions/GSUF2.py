...
from previous_versions.UF import *
...
class GoState:
        ...
        for i in a.children:
            ...
            if i.x>0:
                if len(Find(self.board[i.x-1][i.y]).liberty)==1:
                    tocheck.append((Find(self.board[i.x-1][i.y]).liberty[0].x,Find(self.board[i.x-1][i.y]).liberty[0].y))
                #extend lib
            ...
            if i.y>0:
                if len(Find(self.board[i.x][i.y-1]).liberty)==1:
                    tocheck.append((Find(self.board[i.x][i.y-1]).liberty[0].x,Find(self.board[i.x][i.y-1]).liberty[0].y))
    ...
    def GetMoves(self):
        if self.lastpass==True:
            ...
            if self.playerJustMoved==2:
                a=copy.deepcopy(self.move1)
            else:
                a=copy.deepcopy(self.move2)
            a.append((-1,-1))
